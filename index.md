...
East Asian music, Chinese music in particular, not only has a distinctive style due to its use of pentatonic tonality, but also has accumulated a large number of pieces of music that have been handed down to the present day in its long history. The self-built dataset \emph{China-MIDI} used in this experiment contains 2,708 traditional Chinese instrumental pieces (consisting of a total of 210,326 notes) in MIDI format. With a default tempo setting of $bpm=60$, the total playing time of all the pieces in the dataset is approximately 34.8 hours.

Statistics on the musical information of the \emph{China-MIDI} dataset are given in the figure shown below. For presentation purposes, key signatures are expressed as the number of sharps, while note pitches are expressed as the corresponding MIDI pitches (rest is 0), and chords are split into separate notes. The \emph{China-MIDI} dataset is scheduled to be published in the near future, to bring more types of music data to the field of music generation.
